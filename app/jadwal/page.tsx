...
function generatePrintHtml(p: {
  ...
  const {
    ...
    daftarGuru, daftarMapel, daftarRombel, daftarKelasGabungan, daftarTingkat, daftarLembaga,
...
function generatePrintHtmlGuru(p: {
  ...
  // Satu kolom hari = SATU sel (bukan grid banyak baris) -- semua nama guru piket
  // hari itu ditumpuk sebagai beberapa baris teks DI DALAM sel yang sama (persis
  // seperti Tabel Jadwal Guru Piket pada unduhan jadwal keseluruhan/unit), supaya
  // tidak ada sel kosong sama sekali walau jumlah guru piket tiap hari berbeda.
  const piketTableRows = `<tr>${hariPiketKolom.map(h => {
    const namaList = getPiketHariGabungan(h)
      .map(entry => {
        const nama = daftarGuru.find((g: any) => g.id === entry.guruId)?.nama
        if (!nama) return null
        const labelUnit = labelUnitDari(entry.lembagaId)
        return labelUnit ? `${nama} <span style="font-size:11px;color:#000">(${labelUnit})</span>` : nama
      })
      .filter((n): n is string => !!n)
    return `<td style="padding:4px 6px;font-size:13px;border:1px solid #000;text-align:center;vertical-align:middle;white-space:normal;word-break:break-word;line-height:1.5">${namaList.map(n => `<div>${n}</div>`).join('') || '-'}</td>`
  }).join('')}</tr>`
...
function generatePrintHtmlKelas(p: {
  ...
  // tidak ada sel kosong sama sekali walau jumlah guru piket tiap hari berbeda.
  const piketTableRows = `<tr>${hariPiketKolom.map(h => {
    const namaList = getPiketGuruIdsHariGabungan(h)
